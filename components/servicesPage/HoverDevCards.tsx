...
const HoverDevCards = ({ service, Icon }: any) => {
  return (
    <SimpleReveal sequence={2}>
      <div className="w-[80vw] h-[40vh] xl:w-[60vw]  xl:h-[60vh] cursor-pointer  rounded-xl border-[1px] border-slate-300  overflow-hidden group bg-white  flex flex-col justify-end relative">
        <Image
          src={`${service.imgUrl}`}
      ...
      <Icon className="mb-2 text-2xl text-violet-600 group-hover:text-white transition-colors relative z-10 duration-300" /> */}
        <div className="absolute w-full hover:relative flex flex-col items-start  bg-gradient-to-t from-black/70 group-hover:from-transparent via-black/50 group-hover:via-white/30 to-transparent p-4 ">
          <h3 className=" text-xl xl:text-2xl mb-2 font-bold text-white relative z-10 duration-300 group-hover:translate-x-[0rem] group-hover:-translate-y-10 w-full">
            {service.title}
          </h3>
        </div>
      </div>
